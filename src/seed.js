...
function seedSampleMatches() {
  ...
  seedRan = (async () => {
    ...
    for (const s of SAMPLE) {
      i++;
      const markets = computeMarkets({ odd_1: s[2], odd_x: s[3], odd_2: s[4], odd_over: s[5], odd_under: s[6] });
      await sql`
        INSERT INTO matches (id, home_team, away_team, commence_time, status, markets)
        VALUES (${'sample-' + i}, ${s[0]}, ${s[1]}, ${daysFromNow(i)}, 'open', ${JSON.stringify(markets)}::jsonb)
